...
PROJECT_ROOT = Path(".")
#RAW_LABELED_DIR = PROJECT_ROOT / "datasets" / "raw_labeled"
...
# Change these filenames if your originals use different names
RAW_LABELED_FILENAMES = {
    #"train": "train_labeled.jsonl",
    #"val": "val_labeled.jsonl",
    #"test": "test_labeled.jsonl",
...
SCENE_FILENAMES = {
    "train": "train_scenes_enriched.jsonl",
    "val": "val_scenes_enriched.jsonl",
    "test": "test_scenes_enriched.jsonl",
}
...
def get_script_id(raw_record: Dict[str, Any]) -> Optional[str]:
    """
    Tries common script id fields.
    Edit here if your raw labeled file uses a different field name.
    """
    candidates = [
        raw_record.get("script_id"),
        raw_record.get("id"),
        raw_record.get("scriptId"),
        raw_record.get("scriptID"),
    ]
    for c in candidates:
...
def get_annotations(raw_record: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    Tries common annotation field names.
    Edit here if your raw labeled file uses a different field name.
    """
    candidates = [
        raw_record.get("error_annotations"),
        raw_record.get("annotations"),
...
def parse_scene_index_from_value(value: Any) -> Optional[int]:
    ...
    - "3"
    - "scene_03"
    - "synthetic_script_0010_scene_03"
    """
    if value is None:
        return None
...
def convert_annotation_to_gt_row(
    ...
    # Format C:
    # {"scene_index": 3, "related_scene_index": 2}
...
def process_split(split: str) -> None:
    ...
    for raw_record in raw_rows:
        ...
        if not script_id:
            print(f"[DEBUG] Skipping row without script id: {raw_record}")
            continue
