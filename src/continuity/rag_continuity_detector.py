...
OUTPUT_FILES_RULE_BASED = [
    "train_error_issues.jsonl",
    "val_error_issues.jsonl",
    "test_error_issues.jsonl",
]
...
class RagContinuityDetector:
    ...
    def LoadDetectedIssuesJsonl(self, issue_detected_file_path):
        detected_issues = []

        with open(issue_detected_file_path, "r", encoding="utf-8") as issue_file:
            for json_line in issue_file:
                if json_line.strip():
                    detected_issues.append(json.loads(json_line))

        return detected_issues
        
    def SaveMergedIssueJsonl(self, issues: list[dict], merged_issue_file_path: str) -> None:
        with open(merged_issue_file_path, "w", encoding="utf-8") as issue_file:
            for issue in issues:
                issue_file.write(json.dumps(issue) + "\n")

    def Load_llama_Instruct_model(self, use_4bit: bool = False):
    ...
    def call_llama_batch(self, prompts: list[str], tokenizer, model, max_new_tokens: int = 1536) -> list[dict | None]:
        inputs = tokenizer(
            prompts,
            return_tensors="pt",
            padding=True,
            truncation=True,
            max_length=2048,
            padding_side="left"  # required for Llama
        ).to("cuda")
        ...
        with torch.inference_mode():
            outputs = model.generate(
                **inputs,
                ...
                do_sample=False,
                pad_token_id=tokenizer.eos_token_id,
                eos_token_id=[tokenizer.eos_token_id] + closing_brace_ids
            )
    ...
    def _extract_first_json(self, text: str) -> str | None:
        """Extract the first complete, valid JSON object from model output."""
        text = text.strip()
            ...
            if ch == "{":
                ...
                if depth == 0:
                    ...
                    try:
                        ...
                        return candidate  # return anyway, let caller handle
        return None
    
    def call_llama(self, prompt: str, tokenizer, model, max_new_tokens: int = 1024) -> dict | None:
        # Llama 3.1 Instruct uses chat template
        print("\nThe beginning of Call Llama method call")
        messages = [
            {
                "role": "system",
                "content": (
                    "You are a script continuity analysis assistant. "
        ...
        with torch.no_grad():
            ...
            #print(input_ids.shape)        # should be torch.Size([1, N])
        ...
        try:
            ...
            print(f"  [JSON ERROR] Could not parse model output: {e}")
            print(f"  Raw output: {raw[:300]}")
            return None

    def BuildRule_LLM_MergePrompt_Concise(self, continuity_issue_id, rule_based_result, llm_based_review) -> str:
        ...
        current_script_id = safe_get(rule_based_result, "script_id")
        has_rule_based_error = safe_get(rule_based_result, "detectors", "rule_based", "has_error", default=False)

        if has_rule_based_error:
            #current_scene_id = rule_based_result["detectors"]["rule_based"]["errors"]["scene_id"]
            #current_scene_index = rule_based_result["detectors"]["rule_based"]["errors"]["scene_index"]
            #related_scene_id = rule_based_result["detectors"]["rule_based"]["errors"]["related_scene_id"] 

            current_scene_id = safe_get(rule_based_result, "detectors", "rule_based", "errors", "scene_id")
            current_scene_index = safe_get(rule_based_result, "detectors", "rule_based", "errors", "scene_index")
            related_scene_id = safe_get(rule_based_result, "detectors", "rule_based", "errors", "related_scene_id")
        else:
        ...
        if llm_based_review:
            #review_scene_id = llm_based_review["detectors"]["rag_llm"]["errors"]["scene_id"]
            #review_scene_index = llm_based_review["detectors"]["rag_llm"]["errors"]["scene_index"]
            #review_related_scene_id = llm_based_review["detectors"]["rag_llm"]["errors"]["related_scene_id"]

            review_scene_id = safe_get(llm_based_review, "detectors", "rag_llm", "errors", "scene_id")
            review_scene_index = safe_get(llm_based_review, "detectors", "rag_llm", "errors", "scene_index")
            review_related_scene_id = safe_get(llm_based_review, "detectrors", "ral_llm", "errors", "related_scene_id")
        else:
            review_scene_id = None
            review_scene_index = None
            review_related_scene_id = None

        return f"""You will receive up to two detections for the same scene — one from a rule-based system and one from an LLM review.
            Reconcile them into a single continuity issue record.
            
            Rules:
            - If both sources agree, confirm the issue with high confidence.
            - If only one source flagged it, include it but lower the confidence.
            - If they conflict, use your judgment and explain briefly in "reconciliation_note".
            - Return ONLY a valid JSON object. No extra text, no Python code, no markdown backticks.
            - Start your response with {{ and end with }}
            
            final_merged_result rules:
            - Always a JSON array, even if it contains only one item.
            - If both detectors found the SAME issue type, merge into ONE object with both in supporting_detectors.
            - If both detectors found DIFFERENT issue types, include BOTH as SEPARATE objects in the array.
            - If neither detector found an issue, return one object with has_error: false, all other fields null, supporting_detectors: [].
            - Never omit an issue detected by either detector.
            
            Rule-based detection:
            {json.dumps(rule_based_result, indent=2) if rule_based_result else "null"}
            
            LLM review detection:
            {json.dumps(llm_based_review, indent=2) if llm_based_review else "null"}
            
            IMPORTANT: "continuity_issue_id" MUST be exactly: {continuity_issue_id}. Do not modify or invent any ID fields.
            ...
            {{
                ...
                "detectors": {{
                    ...
                    "rag_llm": {{
                        ...
                        "errors": {{
                            ...
                            "related_scene_id": {review_related_scene_id},
                            "issue_type": string,
                            "severity": string,
                            "description": string,
                            "evidence": string,
                ...
                "final_merged_result": [
                    {{
                        ...
                        "confidence": float (2 decimal places) or null,
                        "supporting_detectors": [string] or []
                    }}
                ]
            }}"""
    ...
    def BuildRule_LLM_Merge_Prompt(self, continuity_issue_id, rule_based_result, llm_based_review) -> str:
        ...
        if llm_based_review:
            ...
            review_scene_id = safe_get(llm_based_review, "detectors", "rag_llm", "errors", "scene_id")
            review_scene_index = safe_get(llm_based_review, "detectors", "rag_llm", "errors", "scene_index")
            review_related_scene_id = safe_get(llm_based_review, "detectors", "ral_llm", "errors", "related_scene_id")
        else:
            review_scene_id = None
            review_scene_index = None
            review_related_scene_id = None
        ...
        return f"""You will receive up to two detections for the same scene — one from a rule-based system and one from an LLM review.
            ...
            IMPORTANT RULES for building final_merged_result:
            - final_merged_result must ALWAYS be a JSON array, even if it contains only one item.
            - If the rule-based result and LLM review result describe the SAME type of continuity issue, 
              merge them into ONE object with supporting_detectors containing both "rule_based" and "llm_review".
            - If the rule-based result and LLM review result describe DIFFERENT continuity issues, include BOTH as SEPARATE objects in the array. Do NOT collapse them into one.
            - If neither detector found any issue, return a single object with has_error set to false and all other fields set to null, and supporting_detectors as an empty array.
            - Never omit an issue that was detected by either detector.
            ...
            {{
                ...
                "detectors": {{
                    "rule_based": {{
                        "has_error": print true if there are rule based errors, false if not
                        "errors": print the following fields if there are rule based errors
                        {{
                            "scene_id": {current_scene_id},
                            "scene_index": {current_scene_index},
                            "related_scene_id": {related_scene_id},
                            "issue_type": print the rule based issue type here,
                            "severity": print the rule based issue severity level here,
                            "description": print the rule based issue description here
                            "evidence": print the rule based issue evidence here
                            "confidence": print the rule based issue confidence level here. it must be number rounded two decimal place
                        }}
                    }},
                    "rag_llm": {{
                        "has_error": print true if there are rag llm review based errors, false if not
                        "errors": print the following fields if there are rag llm review errors
                        {{
                            "scene_id": {review_scene_id},
                            "scene_index": {review_scene_index},
                            "related_scene_id": {review_related_scene_id},
                            "issue_type": print the llm review continuity issue here,
                            "severity": print the llm review severity level here,
                            "description": print the llm review description here
                            "evidence": print the llm review evidence here
                            "confidence": print the llm review confidence level here. it must be number rounded two decimal place
                        }}
                    }},
                    "fine_tuned_llm": {{
                        "has_error": false,
                ...
                "final_merged_result": [
                    {{
                        "has_error": print true if there is final merged issue or false,
                        "issue_type": print the final merged continuity issue type here,
                        "severity": print the final merged issue severity level here,
                        "description": print the final merged issue description here,
                        "evidence": print the final merged issue evidence here,
                        "confidence": print the final merged issue confidence level here. it must be number rounded two decimal place,
                        "supporting_detectors": print the list of supporting detectors of continuity issue here
                    }}
                ]
            }}
            """       
    ...
    def BuildContinuityPrompt_RAG_Rule_Llama_Merge(self, current_scene, previous_scene, continuity_issue_id):
        # current_scene_text = self.BuildSceneDocumentText(current_scene)
        ...
        print("\nContinuity Issue Id (In Llama Merge Method): ", continuity_issue_id)
        
        prompt = f"""
        You are a movie script supervisor Merger Using Llama.
