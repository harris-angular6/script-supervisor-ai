...
PROJECT_ROOT = Path(".")
INPUT_DIR = PROJECT_ROOT / "datasets" / "scenes_enriched"
OUTPUT_DIR = PROJECT_ROOT / "datasets" / "issues"

INPUT_FILES = [
    "train_scenes_enriched.jsonl",
    "val_scenes_enriched.jsonl",
...
# Props that are often portable/personal and useful for continuity checks
HIGH_VALUE_PROPS = {
    "phone", "mug", "bag", "purse", "briefcase", "letter", "resume",
    "paper", "glasses", "hat", "coat", "jacket", "key", "wallet", "bottle"
}
...
def scene_link_strength(prev_scene: Dict[str, Any], curr_scene: Dict[str, Any]) -> str:
    """
    Return rough link strength between adjacent scenes.
    """
...
def detect_character_presence_continuity(
    ...
    # Avoid over-flagging when large cast scenes change naturally
...
def detect_time_of_day_continuity(
    ...
    # If current scene explicitly says LATER, do not flag
    ...
    if prev_rank is not None and curr_rank is not None:
        ...
        # If unknown markers but different and scenes are strongly linked, still flag lightly
...
def detect_location_chronology_continuity(
    ...
    # Stronger signal if marked continuous but location changes
...
def process_file(input_path: Path, output_path: Path) -> None:
    ...
    for script_id, scenes in grouped.items():
