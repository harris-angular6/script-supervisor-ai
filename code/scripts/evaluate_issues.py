...
# Uncomment this line in scripts directory
#from src.evaluation.issue_evaluator import main

PROJECT_ROOT = Path(".")
PRED_DIR = PROJECT_ROOT / "datasets" / "issues_error"
GT_DIR = PROJECT_ROOT / "datasets" / "ground_truth"
REPORT_DIR = PROJECT_ROOT / "reports" / "evaluation_error"

SPLITS = ["train", "val", "test"]

PRED_FILENAMES = {
    "train": "train_error_issues.jsonl",
    "val": "val_error_issues.jsonl",
    "test": "test_error_issues.jsonl",
}

GT_FILENAMES = {
    "train": "train_ground_truth_issues.jsonl",
    "val": "val_ground_truth_issues.jsonl",
    "test": "test_ground_truth_issues.jsonl",
}
...
def issue_key(issue: Dict[str, Any]) -> Tuple[str, str, str, str]:
    """
    Exact-match key for evaluation.
    """
    return (
...
def build_markdown_report(split: str, results: Dict[str, Any]) -> str:
    ...
    lines: List[str] = []
    lines.append(f"# Synthetic Error Evaluation Report - {split}")
    lines.append("")
    ...
    lines.append("- Predictions come from the synthetic-error issue detector.")
    lines.append("- Ground truth comes from synthetic error annotations generated during error injection.")
    lines.append("")
...
def main() -> None:
    ...
    for split in SPLITS:
        pred_path = PRED_DIR / PRED_FILENAMES[split]
        gt_path = GT_DIR / GT_FILENAMES[split]

        pred_rows = read_jsonl(pred_path)
        gt_rows = read_jsonl(gt_path)

        print(f"[DEBUG] Split={split}")
        print(f"[DEBUG] Loaded predictions={len(pred_rows)} from {pred_path}")
        print(f"[DEBUG] Loaded ground_truth={len(gt_rows)} from {gt_path}")

        if not gt_rows:
            print(f"[WARNING] No ground truth rows for split '{split}'. Skipping.")
            continue
